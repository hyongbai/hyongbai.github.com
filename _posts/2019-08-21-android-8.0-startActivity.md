...
可以看到通过token对象拿到`ActivityRecord`，可以判定这次就是我们之前正在`pausing`的Activity，然后在`completePauseLocked`中又回到了上面熟悉的`resumeFocusedStackTopActivityLocked`函数。

而这次调用`resumeFocusedStackTopActivityLocked`函数时，由于前面在startPausingLocked时已经将mResumedActivity置`null`，此时会跳过`startPausingLocked`分支逻辑，而执行`startSpecificActivityLocked`。
